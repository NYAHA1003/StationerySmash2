...
namespace Main.TextSetting
{
    ...
    public class LocalizeText : MonoBehaviour
    {
        ...
        string Localize(string key) //어떤 문자열이 필요하진 key를 매개변수로 받는다 
        {
            ...
            return LanguageSingleton.Instance.Langs[LanguageSingleton.Instance.curLangIndex].value[keyIndex];      //현재 언어 인덱스, value의 key를 기준으로 문자열을 반환한다. 
        }
        ...
        void LocalizeChanged()
        {
            if (tmpGUI != null)
            {
                tmpGUI.text = Localize(textKey);  // Localize의 반환 값으로 텍스트를 바꿔준다.
                return;
            }
